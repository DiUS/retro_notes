...
links.Timeline.prototype.repaintAxisCharacters = function () {
    ...
    if (!axis.characterMinor) {
        ...
        characterMinor.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
    ...
    if (!axis.characterMajor) {
        ...
        characterMajor.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
...
links.Timeline.prototype.repaintAxisHorizontal = function() {
    ...
    if (hasAxis) {
        if (!axis.backgroundLine) {
            ...
            backgroundLine.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
    ...
    if (hasAxis) {
        if (axis.line) {
            ...
            line.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
            line.style.left = "0px";
...
links.Timeline.prototype.repaintAxisMinorText = function (x, text) {
    ...
    if (index < minorTexts.length) {
        ...
        label.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
...
links.Timeline.ItemBox.prototype.createDOM = function () {
    ...
    var divContent = document.createElement("ARTICLE");
    ...
    divDot.style.width  = links.Timeline.DOT_DIMENSIONS.width;
    divDot.style.height = links.Timeline.DOT_DIMENSIONS.height;
    divBox.dot = divDot;
...
links.Timeline.ItemBox.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        if (axisOnTop) {
            ...
            dot.style.top = (axisHeight - this.dotHeight/2) + links.Timeline.DOT_DIMENSIONS.topOffset + "px";
        }
        else {
...
links.Timeline.ItemDot.prototype.createDOM = function () {
    ...
    // dot at start
    var divDot = document.createElement("DIV");
    divDot.style.position = this.state.timelineAxisFixed ? "fixed" : "absolute";
...
links.Timeline.StepDate.prototype.getLabelMinor = function(options, date) {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.HOUR:
            return date.getHours() + ":" + this.addZeros(date.getMinutes(), 2);
